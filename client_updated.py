...
#
# Chunks arrive already silence-trimmed by the server, so we butt them
# together with a short equal-power crossfade. speaker_gap_ms=0 => fully
# continuous audio. Raise it only if you want turn-taking pauses back.
# =============================================================================
...
def crossfade_concat(
    ...
    out = np.asarray(chunks[0][0], dtype=np.float32)

    for audio, is_cont in chunks[1:]:

        audio = np.asarray(audio, dtype=np.float32)

        if (not is_cont) and speaker_gap_ms > 0:
            gap = np.zeros(
                int(sr * speaker_gap_ms / 1000),
                dtype=np.float32,
            )
            out = np.concatenate([out, gap, audio])
            continue

        n = min(
            int(sr * xfade_ms / 1000),
            len(out),
            len(audio),
        )
        ...
        if n > 0:
            ...
            out[-n:] = out[-n:] * fade_out + audio[:n] * fade_in
            out = np.concatenate([out, audio[n:]])
        else:
...
def run_production(
    ...
    # Collect chunks, then concatenate once (gapless) at the end.
    collected = []
